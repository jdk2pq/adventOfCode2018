...
export class Unit {
    ...
    identifyInRangeTargets = (setCombat: boolean) => {
        ...
        this.inRangeTargets = [];
        if (this.enemy === 'E') {
            targets = workingElves;
        } else {
            targets = workingGoblins;
        }
        if (targets.length > 0) {
    ...
    attack = () => {
        ...
        if (chosenTargetToAttack.hp <= 0) {
            ...
            if (this.enemy === 'E') {
                ...
                // console.log(goblins);
                workingGoblins = workingGoblins.filter(elf => elf.id !== chosenTargetToAttack.id);
                // console.log(goblins);
            }
        }
    }
...
const runBattle = () => {
    readBoard();
    workingElves = elves;
    workingGoblins = goblins;
    while (combatInProgress) {
        printBoard();
        let combatBrokeIter = false;
        workingElves.concat(workingGoblins).sort((u1: Unit, u2: Unit) => {
            ...
            if (workingElves.concat(workingGoblins).map(unit => unit.id).includes(unit.id)) {
                unit.startTurn();
            }
            if (!combatInProgress) {
    ...
    if (elfDied) {
        ...
        elfDied = false;
        elfAttackPower++;
        console.log(`elf attack power increased to ${elfAttackPower}`);
        iter = 0;
        runBattle();
    }
};
runBattle();
const hpReduced = workingElves.concat(workingGoblins).reduce((acc: number, unit: Unit) => {
    return acc + unit.hp;
}, 0);
console.log(iter);
