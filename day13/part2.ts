    ...
    carts.forEach((cart: Cart) => {
        if (carts.find((c) => c.id === cart.id)) {
            ...
            if (nextVal.direction) {
                ...
                cart.crashed = true;
                track[nextLocation[0]][nextLocation[1]] = nextVal.savedValue;
            ...
            if (!cart.crashed) {
                ...
                console.log(`# of carts to start with: ${carts.length}`);
                console.log(cart.id);
                console.log(nextVal.id);
                carts = carts.filter((c) => c.id !== cart.id);
                carts = carts.filter((c) => c.id !== nextVal.id);
                console.log(`# of carts left: ${carts.length}`);
            }
        }
    });
};
...
console.log(carts[0].currentLocation);
// printTrack();
console.log(iter);
